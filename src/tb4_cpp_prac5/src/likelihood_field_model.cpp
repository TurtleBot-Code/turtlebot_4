...
namespace iar_amcl
{
    ...
    LikelihoodFieldModel::sensorFunction(nav2_amcl::LaserData * data, pf_sample_set_t * set)
    {
        ...
        for (j = 0; j < set->sample_count; j++) {
                ...
                */
     				 pf_vector_t Tsens = self->laser_pose_;
     				 
     				 xs = pose_robot.v[0] + Tsens.v[0]*cos(pose_robot.v[2]) - Tsens.v[1]*sin(pose_robot.v[2]);
     				 
     				 ys = pose_robot.v[1] + Tsens.v[0]*sin(pose_robot.v[2]) + Tsens.v[1]*cos(pose_robot.v[2]);
     				 
     				 thetas = pose_robot.v[2] + Tsens.v[2]; 
                ...
                for (i = 0; i < data->range_count; i += step) {
                    ...
                    */
                    		...
                    		hit_x = xs + obs_range * cos(thetas + obs_bearing);
                    		hit_y = ys + obs_range * sin(thetas + obs_bearing);
                    		...
                    		pz += self-> z_hit_ * exp ((-1.0 * dist *dist) / (2 * self-> sigma_hit_ * self-> sigma_hit_) );
                    }
